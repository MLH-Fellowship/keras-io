...
We will be running the model training for 10 epochs. A RNN (recurrent neural network)
is used to record sequences and track the pattern. Since the timeseries data is nothing
but a long sequence of values, RNN will track and find patterns from this. LSTM is a type
of RNN which solves the problem of vanishing gradients and is well suited for our problem.
GRU is another type of RNN and works very similar to the LSTM.
...
"""
Saving a checkpoint will allow us to load the weights from a trained model so that we
don't have to train the model again and again whenever new runtime is started.
"""
